...
class DoublesEventScoreTracker:
    def __init__(self, mini_court):
        """
        Initialize the score tracker with the mini court for doubles.
        mini_court: instance of MiniCourt class, used to detect fouls based on mini court boundaries.
        """
        self.mini_court = mini_court
        self.team_scores = {1: 0, 2: 0}  # Points for each team (Team 1 and Team 2)
        self.game_scores = {1: 0, 2: 0}  # Games won by each team
        self.set_scores = {1: 0, 2: 0}  # Sets won by each team
        ...
        self.winning_margin_in_games = 2  # Team must win a set by 2 games

    def update_score(self, winner_team):
        """Update the score based on the winner of the point."""
        if self.team_scores[winner_team] == 0:
            self.team_scores[winner_team] = 15
        elif self.team_scores[winner_team] == 15:
            self.team_scores[winner_team] = 30
        elif self.team_scores[winner_team] == 30:
            self.team_scores[winner_team] = 40
        elif self.team_scores[winner_team] == 40:
            # Team wins the game
            self.game_scores[winner_team] += 1
            self.team_scores = {1: 0, 2: 0}  # Reset after game win
            self._check_set_winner(winner_team)

    def _check_set_winner(self, game_winner):
        """Check if a team has won the set."""
        other_team = 2 if game_winner == 1 else 1

        # Check if the team has enough games to win the set
        if (self.game_scores[game_winner] >= self.games_to_win_set and 
            (self.game_scores[game_winner] - self.game_scores[other_team]) >= self.winning_margin_in_games):
            self.set_scores[game_winner] += 1  # Team wins the set
    ...
    def detect_foul(self, ball_position, player_hit, net_x, player_positions):
        """
        Detect fouls based on the ball's position on the mini court for doubles.
        ...
        if net_hit:
            ...
            return "net_hit", self._get_other_team(hitting_team)
        ...
        if out_of_bounds:
            self.foul_detected = True
            return "out_of_bounds", self._get_other_team(hitting_team)

        return None, None  # No foul detected

    def _is_ball_out_of_bounds(self, ball_position, player_hit, net_x):
        """Check if the ball is out of bounds using mini court boundaries in doubles."""
        ball_x, ball_y = ball_position

        # Ensure ball has crossed the net and is in the opponent's half
        if (player_hit in [1, 2] and ball_x <= net_x) or (player_hit in [3, 4] and ball_x >= net_x):
    ...
    def _is_net_hit(self, ball_position, player_hit, net_x):
        """Check if the ball has hit the net based on its position relative to the net on the mini court in doubles."""
        ball_x, ball_y = ball_position
        if player_hit in [1, 2] and ball_x > net_x:
            return False  # Team 1 hit, ball crossed the net
        if player_hit in [3, 4] and ball_x < net_x:
            return False  # Team 2 hit, ball crossed the net

        # If the ball hasn't crossed the net after being hit
        return True

    def _get_other_team(self, hitting_team):
        """Get the opposing team."""
        return 2 if hitting_team == 1 else 1
    ...
    def draw_score_on_frame(self, frame):
        """Draw the current score for both teams on the frame."""
        ...
        # Display Team 1 and Team 2 points, games, and sets
        cv2.putText(frame, f"Team 1: {self.team_scores[1]} pts | {self.game_scores[1]} games | {self.set_scores[1]} sets", 
                    (20, frame_height - 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Team 2: {self.team_scores[2]} pts | {self.game_scores[2]} games | {self.set_scores[2]} sets", 
